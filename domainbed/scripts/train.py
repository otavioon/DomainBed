...
def to_serializable(obj):
    """
    Recursively convert numpy, torch and other non-serializable types 
    in a dictionary (or any nested structure) to native Python types.
    """
    if isinstance(obj, dict):
        ...
        return obj.tolist()  # or obj.item() if it's a scalar
    else:
        return obj
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Domain generalization",
    ...
    for step in tqdm.tqdm(
        range(start_step, n_steps), desc="Training steps", total=(n_steps-start_step), unit="steps"
    ):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            with open(epochs_path, "a") as f:
                ...
                f.write(json.dumps(serializable_results, sort_keys=True) + "\n")
